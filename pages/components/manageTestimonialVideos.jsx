...
export default function ManageTestimonialVideos() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const storagePath = `testimonial-videos/${Date.now()}-${safeStorageFileName(result.fileName)}`;
  ...
  const handleDelete = async (video) => {
    const ok = window.confirm(
      `Delete testimonial video "${video.title || video.fileName}"?`
    );
    if (!ok) return;
    ...
    <div className="w-full space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Upload videos for the customer app. Files over 10 MB are compressed
          first (any source size). If compression fails, only originals up to 20
          MB are accepted.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="ui-card-padded space-y-4">
        ...
        <div>
          <label htmlFor="videoTitle" className="ui-label">
            Title (optional)
          </label>
          <input
            id="videoTitle"
            ...
            placeholder="e.g. Customer success story"
            className="ui-input mt-1"
          />
        </div>

        <div>
          <label htmlFor="videoFile" className="ui-label">
            Video file
          </label>
          <input
            id="videoFile"
      ...
      <div className="space-y-4">
        ...
        {videos.length === 0 ? (
          <div className="ui-empty-state">No testimonial videos yet.</div>
        ) : (
          videos.map((video) => (
            <div
              key={video.id}
